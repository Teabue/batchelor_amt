...
for fourier_folder in os.listdir(run_folder_name):
    ...
    for loss_folder in os.listdir(run_fourier):
        ...
        for song in songs:
            ...
            all_paths = [[os.path.exists(os.path.join(aud_path, f"{song}{ext}")) for ext in configs['preprocess']['audio_file_extensions']] for aud_path in audio_dirs]
            avail_paths = np.any(all_paths, axis = 1)
            if not np.any(avail_paths):
                raise FileNotFoundError(f"{song} could not be found")
            avail_ext = np.asarray(configs['preprocess']['audio_file_extensions'])[np.any(all_paths, axis = 0)][0]
